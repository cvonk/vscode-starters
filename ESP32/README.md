...
### Start simple
 ...
 - Start compile/flash/monitor
   - [F1] » ESP-IDF: Build, flash and monitor (`ctrl-e` `d`)(https://docs.espressif.com/projects/esp-idf/en/latest/api-guides/tools/idf-monitor.html).
     ...
     - Error: `no such option: -b`.  Change `idf.py monitor` line in `.vscode\tasks.json` so that the command `monitor` is behind the options.
...
Fix `esp-idf-master/tools/cmake/build.cmake`
...
cd esp-idf-master/
set IDF_TOOLS_PATH=C:/Users/YOURNAME/espressif/bin-master
.\install.bat
```
